...
def process(inputfile, outputdir):
    ...
    for line in open(inputfile, encoding="latin-1"):
        ...
        if len(strippedline) == len(line):
            if VERBOSE:
                print(f"{inputfile}: Missing end-of-line: {line}")
            badlinecount += 1
            continue
        ...
        if toolongfield:
            if VERBOSE:
                print(f"{inputfile}: line presumed bad: {line}")
            badlinecount += 1
            continue
        goodlinecount += 1
        # Write line
    ...
    print(
        f"{inputfile}: {fraction*100:.0f}% recovered, {goodlinecount} lines of {goodlinecount+badlinecount}, written to {outputfile}"
    )
